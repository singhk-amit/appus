...
```
    To view structured documentation, run the command
...
```
A personal API token can be generated by going to your Settings -> Access Tokens and in the Personal Access Tokens section, check the read_repository checkbox and generate a token that you need to substitute for 'your_personal_api_token'
and then install the library:
```
composer require adminn/admin
```
Next, run the command in the console or skip this step and automate this process by adding event handlers described below in the scripts section
```
php artisan admin:install
```
this will add styles and js scripts to the public folder, and the configuration file ```admin.php``` will be added to the config folder.
To further automate the updating of styles and scripts when updating the package, it is necessary to add the removal and reinstallation of styles and package scripts to the composer.json in scripts and the post-dump-autoload event:
```json
{
    "scripts": {
        "post-autoload-dump": [
...
```
This will add a controller that inherits from AdminController that implements all the CRUD methods for your resource.
You can override these methods in your controller.
...
All relations for the table, detailed page and form are formed in the field names with enumeration separated by a dot.
```php
...
```
HasMany, HasOne, BelongsTo, and BelongsToMany are available for the table and detail page. For form only HasOne.
For the form, IуlongsToMany is a separate field (for more details, see the "Relationships" section in the forms).

#### Переопределение шаблонов

To override templates, you need to create a folder in the vendor/admin views folder and add a view with the same template name as you want to override.
For example, to override the template ```admin/themes/default/layouts/menu.blade.php```, add the file ```resources/views/vendor/admin/layouts/menu.blade.php```.

#### Menu

To add menu items, you need to create a file (for example, App/Menu/menu.php), and then add the path to this file to the menu array in config/admin.php
```php
    'menu' => [
...
After creating a file to add a menu item, you need to call the add() method from the Appus\Admin\Services\Menu\Facades\Menu facade or via the Menu alias:
```php
use Appus\Admin\Services\Menu\Facades\Menu;

Menu::add('Users');
```
Methods to customize the menu:

- add(string $name, bool $topMenu) - adds a menu item. If $topMenu is set to true, then the menu item will be added to the header

- order(int $order) - sets the order of the menu item

- icon(string $classes) - sets the icon for the menu item and accepts the flaticon class or space-separated classes for fontawesome

- route(string $routeName, array $params, bool $absoluteUrl) - sets the link for the menu item

- sub(Closure $callback) - adds sub-items to the menu item

- actions(array $actions) - you can add a list of routes that are included in this resource to display the active menu item. Controller names, action names, resource name or specific route name are supported

- minimizeText(bool $value) - can add text at the bottom of each navigation item when the navigation is minimized

- if(Closure $callback) - adds a condition for drawing a menu item
...
To add another resource of the same type,
for example, on the detailed page of the resource, add another card with detailed information,
you must call the card(\Closure $callback) method, which accepts a closure whose argument is a new resource instance, and this anonymous function must return the configured resource.
The resource is configured in exactly the same way as the resource outside of this method.
